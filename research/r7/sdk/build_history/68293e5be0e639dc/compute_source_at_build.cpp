...
int main(int argc,char** argv) {
  ...
  try {
    ...
    for(int i=0;i<repetitions;++i) {
      ...
      t=Clock::now();auto run=kernel(uint64_t{1},abo,wbo,cbo,d1,ibo,uint32_t(instr.size()/4),d2);run.wait2(std::chrono::milliseconds(30000));const auto elapsed=seconds(t);
      ...
      std::cout<<"{\"stage\":\"result\",\"iteration\":"<<i<<",\"state\":"<<run.state()<<",\"host_launch_wait2_s\":"<<elapsed<<",\"sync_from_device_s\":"<<sync_s<<",\"launch_to_output_sync_s\":"<<elapsed+sync_s<<",\"compared_int32\":2048,\"mismatches\":"<<mismatches<<",\"inputs_unchanged_host_mapping\":"<<(unchanged?"true":"false")<<",\"output_sha256\":\""<<sha256(cp,expected.size())<<"\",\"numeric_pass\":"<<(!mismatches&&unchanged?"true":"false")<<"}\n";
