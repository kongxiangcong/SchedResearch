...
def simulate(graph, hw, environment, detailed=False, intervention=None, causal=False):
    ...
    while len(completed) < len(nodes):
        ...
        if not external_busy and any(ext_queues):
            ...
            if causal and not applied and 64 <= decision_count <= 67:
                cost = 2.0
                other = 1 - chosen_cluster
                if (packets[default]['direction'] == 'dma_read' and local_busy[chosen_cluster]
                        and not local_busy[other] and ext_queues[other]
                        and packets[ext_queues[other][0]]['direction'] == 'dma_read'):
                    chosen = ext_queues[other][0]
                    chosen_cluster = other
                    cost += 8.0
                    applied = True
